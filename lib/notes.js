...
function getNotes (userid, offset, limit, callback) {
  models.Note.findAll({
    where: {
      [Op.or]: [{ ownerId: userid }, { permission: { [Op.ne]: 'private' } }]
    },
...
function notesGet (req, res) {
  if (req.isAuthenticated()) {
    getNotes(req.user.id, req.query.offset, req.query.limit, function (err, notes) {
