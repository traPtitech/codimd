...
// auth method
config.isTrapEnable = config.trap.pubkey && config.trap.cookieName
...
if (['filesystem', 's3', 'minio', 'imgur', 'azure', 'lutim', 'swift'].indexOf(config.imageUploadType) === -1) {
  logger.error('"imageuploadtype" is not correctly set. Please use "filesystem", "s3", "minio", "azure", "lutim", "swift" or "imgur". Defaulting to "filesystem"')
